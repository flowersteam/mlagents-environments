...
class UnityEnv(gym.Env):
    ...
    def __init__(self, environment_filename: str, worker_id=0, use_visual=False, multiagent=False):
        ...
        self.reset_dict = self._env._resetParameters
    ...
    def reset(self):
        ...
        for key, value in self.reset_dict.items():
    ...
    def reset_param(self, key, value):
        if key in self.reset_dict.keys():
            self.reset_dict[key] = value
            logger.warn(str(key) + ": " + str(value) + " will apply on next reset.")
            return

        else:
            logger.warn("The key:" + str(key) + " does not exist.")
